#Clip Classifier File Using Domain Prompt
# Idea for Domain-based Classifier
# Keep 3 tokens, one token is connected to the domains via a linear layer and make it trainable, all the domains label encoded
...
class ClipClassifier(nn.Module):
    def __init__(self, settings,  clip_model=None):
        ...
        self.prompt = clip.tokenize(self.ctx_init).cuda()
        with torch.no_grad():
    ...
    def construct_soft_prompt(self, classnames, clip_model, domains):
        ...
        ctx_d = self.dom(torch.tensor(domains).cuda().type(clip_model.dtype))
        if ctx_d.dim() == 2:
    ...
    def forward(self,qimage_clip_processed, qtext_clip_tokenized, caption_u,domain):
        ...
        prompt_learn,tokenized_prompts =  self.construct_soft_prompt(caption_u,self.clip,domain)
